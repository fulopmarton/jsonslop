  ...
  <div class="search-bar flex items-center space-x-2 p-3 border-b"
    ...
    <div class="flex-1 relative">
      <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
        <svg class="h-4 w-4" style="color: var(--text-tertiary);" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
        </svg>
      </div>
      <input ref="searchInput" v-model="searchQuery" type="text" placeholder="Digging through the slop..." :class="[
      ...
      <button v-if="searchQuery" @click="clearSearch"
        class="ml-2 absolute inset-y-0 right-0 pr-3 flex items-center transition-colors duration-200 hover-lift"
    ...
    <div v-if="hasSearchResults" class="flex items-center space-x-2 text-sm" style="color: var(--text-secondary);">
      <span class="whitespace-nowrap font-medium ml-2">
    ...
    <div v-else-if="searchQuery && !hasSearchResults" class="ml-2 text-sm font-medium"
