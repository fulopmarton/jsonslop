  ...
  <div ref="containerRef" class="graph-canvas" :style="{ width: `${width}px`, height: `${height}px` }">
    <svg ref="svgRef" :width="width" :height="height" class="graph-svg" @wheel="zoom.handleWheel"
      ...
      <g class="zoom-container" :transform="zoom.transformString.value">
        <!-- Links -->
        <g class="links-group">
          <path v-for="link in layout.links.value" :key="`${getNodeId(link.source)}-${getNodeId(link.target)}`"
            class="link" :d="getLinkPath(link)" :stroke="getLinkStroke(link)" :stroke-width="getLinkStrokeWidth(link)"
        ...
        <g class="nodes-group">
          <GraphNodeComponent v-for="node in layout.nodes.value" :key="node.id" :node="node"
            :is-selected="isNodeSelected(node.id)" :is-highlighted="isNodeHighlighted(node.id)" :show-labels="true"
            @click="handleNodeClick" @double-click="handleNodeDoubleClick" @context-menu="handleNodeContextMenu"
            @drag-start="handleNodeDragStart" @drag="handleNodeDrag" @drag-end="handleNodeDragEnd"
            @focus="handleNodeFocus" @blur="handleNodeBlur" @tooltip-show="handleTooltipShow"
            @tooltip-hide="handleTooltipHide" />
        </g>

        <!-- Tooltips layer - rendered on top of all nodes -->
        <g class="tooltips-group">
          <foreignObject v-if="tooltipNode" :x="tooltipX" :y="tooltipY" :width="tooltipWidth" :height="tooltipHeight"
